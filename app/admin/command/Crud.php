        ...
        if ($relation) {
            ...
            foreach ($relationArr as $index => $relationTable) {
                ...
                $relations[] = [
                    ...
                    //关联表外键
                    'relationForeignKey'    => isset($relationForeignKey[$index]) ? $relationForeignKey[$index] : parseName($relationName).'_id',
                    //关联表主键
            ...
            foreach ($columnList as $k => $v) {
                ...
                if ($v['DATA_TYPE'] != 'text' && $inputType != 'fieldlist') {
                    ...
                    if ($this->headingFilterField && $this->headingFilterField == $field && $itemArr) {
                        $headingHtml = $this->getReplacedStub('html/heading-html', ['field' => $field, 'fieldName' => parseName($field, 1, false)]);
                    }
