...
class Auth
{
    ...
    public function sso_attach()
    {
        $this->sso_config = get_addon_config('sso');
        $this->sso_borker = new Broker($this->sso_config['sso_server'], $this->sso_config['sso_broker_id'], $this->sso_config['sso_broker_secret']);
        $this->sso_borker->attach(true);
    }
    ...
    {
        try {
            $info = $this->sso_borker->login($account, $password);
     ...
     */
    public static function sso_login($account, $password)
    {
        ...
        try {
            ...
            return ValidationResult::error($exception->getMessage());
        }
        if (!$user) {
            return ValidationResult::error('Account is incorrect');
        }
        if ($user->status != 'normal') {
            return ValidationResult::error('Account is locked');
        }
        if ($user->password != md5(md5($password).$user->salt)) {
            return ValidationResult::error('Password is incorrect');
        }
